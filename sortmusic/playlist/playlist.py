...
def write_m3u(
    ...
    with output_path.open("w", encoding="ascii", errors="replace") as fh:
        if extended:
            fh.write("#EXTM3U\n")
            # FIX 8: plain strings — no f-prefix needed when there's no interpolation
        ...
        for track in tracks:
            ...
            if extended:
                ...
                fh.write(f"#EXTINF:{track['duration']},{track['artist']} - {track['title']}{pop_tag}\n")
            fh.write(f"{path_str}\n")
...
def write_m3u8(
    ...
    for non-ASCII filenames and metadata.
    ...
    with output_path.open("w", encoding="utf-8") as fh:
        ...
        for track in tracks:
            ...
            fh.write(
                f"#EXTINF:{track['duration']},"
                f"{track['artist']} - {track['title']}{pop_tag}\n"
            )
            fh.write(f"{path_str}\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # M3U8 playlist (UTF-8, recommended)
